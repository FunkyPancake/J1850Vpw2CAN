...
constexpr std::array<uint8_t, 256> Vpw::crcTable_ = {
...
uint32_t Vpw::OnTimerEvent(uint32_t status) {
    ...
    if (status & kFTM_Chnl2Flag) {
        eventsToClear |= kFTM_Chnl2Flag;
    }
    if (status & kFTM_Chnl3Flag) {
        static uint32_t _lastEdgeCntrVal{0};
        uint32_t counterValue = FTM3->CONTROLS[3].CnV;
        uint32_t pulseLength = counterValue - _lastEdgeCntrVal;
